...
aws s3 cp "$RELEASE_ARCHIVE" "s3://$RELEASE_BUCKET/$RELEASE_S3_OBJECT"
...
aws s3 cp "$RELEASE_DIR/abq" "s3://$RELEASE_BUCKET/$NEW_RELEASE_S3_OBJECT"
aws s3api put-object-tagging \
  --bucket "$RELEASE_BUCKET" \
...
aws s3 cp "$RELEASE_DIR/abq_tester_harness" "s3://$RELEASE_BUCKET/$NEW_RELEASE_S3_OBJECT_TESTER_HARNESS"
aws s3api put-object-tagging \
  --bucket "$RELEASE_BUCKET" \
