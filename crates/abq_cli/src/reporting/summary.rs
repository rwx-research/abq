...
impl SuiteTracker {
    ...
    pub fn new(manifest: Vec<TestSpec>) -> Self {
        Self {
    ...
    pub fn exit_code(&self) -> ExitCode {
        self.load_aggregated_metrics().exit_code
    }
...
mod test {
    ...
    use crate::reporting::{summary::SuiteTracker, SuiteResult};

    use super::{AggregatedMetrics, OverallStatus, StatusTag};
    ...
    fn inject_results(
        ...
        let mut tracker = SuiteTracker::new(manifest);
        for (work_id, run_number, result) in results_to_inject {
            tracker.account_result(work_id, run_number, &result);
        }
    ...
    fn update_suite_result_on_change() {
        let (mut tracker, to_wid) = inject_results([
