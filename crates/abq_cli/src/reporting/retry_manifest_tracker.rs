...
impl RetryManifestTracker {
    pub fn new(manifest: Vec<TestSpec>) -> Self {
        let mut entry_indices = HashMap::with_capacity(manifest.len());
        let entries = manifest
            ...
            .map(|(index, spec)| {
                ...
                entry_indices.insert(work_id, index);
...
mod test {
    ...
    fn empty_failing_subset_when_no_failures() {
        ...
        let mut tracker = RetryManifestTracker::new(manifest);
    ...
    fn failing_subset_consists_of_accounted_failures() {
        ...
        let manifest = vec![spec1.clone(), spec2, spec3.clone()];
        let mut tracker = RetryManifestTracker::new(manifest);
    ...
    fn failing_subset_for_subsequent_attempt_consists_of_accounted_failures_only_for_subsequent_attempt(
        ...
        let manifest = vec![spec1.clone(), spec2, spec3.clone(), spec4, spec5.clone()];
        let mut tracker = RetryManifestTracker::new(manifest);
    ...
    fn preserve_order_in_failing_subset() {
        ...
        let mut tracker = RetryManifestTracker::new(manifest.clone());
